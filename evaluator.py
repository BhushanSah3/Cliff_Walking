import gym, cv2
import pickle as pkl  # same to get q table
import numpy as np

# Creating  an environment
cliffEnv = gym.make("CliffWalking-v0")

# Loading Q Table from file  ,file name ,  rb= read binary mode
...
# epsilon-greedy policy function same as sarsa
def policy(state, explore=0.0):
    action = int(np.argmax(q_table[state]))
    if np.random.random() <= explore:
        action = int(np.random.randint(low=0, high=1, size=4))
    return action
...
for episode in range(NUM_EPISODES):
    ...
    frame = initialize_frame()  # intitializing frame
    state = cliffEnv.reset() # intitializing environment

    # For every step of the episode
    while not done:
        # Show the current state to the user
        frame2 = put_agent(frame.copy(), state)   # getting next frame
        cv2.imshow("Cliff Walking", frame2)  #showing frame
        cv2.waitKey(250)

        # Select action according to policy from above  above function
        action = policy(state)

        # Take an action in the environment using step method
